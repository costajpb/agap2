...
export default function TVShow({ details }: TVShowProps) {
    ...
    useEffect(() => {
        if (!!articleRef.current)
            setUseCase(new TVShowUseCase(repository, details.id, new Emitter(articleRef.current)))
    ...
    useEffect(() => {
        ;(async () => {
            if (target && useCase) {
                const episode = (await useCase.current).episodes.find(
                    ({ id }) => id === parseInt(`${target.dataset.episodeId}`)
                )
    ...
    return (
        <x.article
            ...
            ref={articleRef}
        >
            <PageTitle textAlign={{ _: 'initial', md: 'right' }} gridArea="b">
                {details.title}
            </PageTitle>
            <x.div
                gridArea="c"
